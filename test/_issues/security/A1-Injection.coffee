  ...
  it 'Issue 26 - Data_Files.set_File_Data - DoS via file_Contents', ->
    ...
    using new Data_Team(), ->
      ...
      create_File = (size)=>
        ...
        @.set_Team_Data_Json project,file_Name, new_File_Contents   # set team with json data
                  .assert_Is_True()

        file_Path.file_Contents().assert_Is  new_File_Contents      # confirm team was changed
                                 .assert_Size_Is (size + 11)        # to the expected size (+11 from json data field and padding)

      restore_File = ()=>
        original_Content = { "will-be": "changed by tests" }
        @.set_Team_Data_Json project,file_Name, original_Content.json_Pretty()
            .assert_Is_True()
